...
const Donations: React.FC = () => {
  ...
  const fetchDonations = async () => {
    setIsLoading(true);
    try {
      const data = await getDonations(filter !== 'all' ? filter : undefined);
      setDonations(data);
    } catch (error) {
      ...
      <div className="bg-white shadow overflow-hidden sm:rounded-md">
        <div className="px-4 py-5 border-b border-gray-200 sm:px-6">
          <div className="flex flex-wrap items-center justify-between">
            <h3 className="text-lg leading-6 font-medium text-gray-900">Donation Management</h3>
                ...
                <div className="flex items-center justify-between">
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center">
                      ...
                      <span className={`ml-2 px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${getStatusBadgeClass(donation.status)}`}>
                  ...
                  <div className="ml-4 flex-shrink-0 flex">
                    ...
                    {donation.status === 'pending' && (
                      <button
                        onClick={() => navigate(`/admin/donations/${donation.id}/assign`)}
                      ...
                      <button
                        onClick={() => {
                          if (window.confirm('Are you sure you want to cancel this donation?')) {
                            // TODO: Implement cancel donation functionality
                            showToast('info', 'This functionality is not yet implemented');
                          }
                        }}
