...
import { getDonationById, updateDonation, DonationFormData } from '../../services/donations';
...
const EditDonation: React.FC = () => {
  ...
  const fetchDonation = async () => {
    ...
    try {
      ...
      const formattedDonation = {
        ...
        pickup_date: donation.pickup_date ? new Date(donation.pickup_date).toISOString().split('T')[0] : '',
        categories: donation.categories || []
      };
      
      setFormData({
        title: formattedDonation.title,
        description: formattedDonation.description || '',
        quantity: formattedDonation.quantity,
        unit: formattedDonation.unit,
        expiry_date: formattedDonation.expiry_date,
        pickup_address: formattedDonation.pickup_address || '',
        pickup_city: formattedDonation.pickup_city || '',
        pickup_state: formattedDonation.pickup_state || '',
        pickup_zip_code: formattedDonation.pickup_zip_code || '',
        pickup_instructions: formattedDonation.pickup_instructions || '',
        pickup_date: formattedDonation.pickup_date,
        pickup_time_start: formattedDonation.pickup_time_start || '',
        pickup_time_end: formattedDonation.pickup_time_end || '',
        categories: formattedDonation.categories
      });
    } catch (error) {
      showToast('error', 'Failed to load donation');
      navigate('/donations');
    } finally {
      setIsLoading(false);
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setIsSaving(true);
    try {
      await updateDonation(parseInt(id), formData);
      showToast('success', 'Donation updated successfully');
      navigate('/donations');
    } catch (error) {
      showToast('error', 'Failed to update donation');
    } finally {
      setIsSaving(false);
    }
  };
