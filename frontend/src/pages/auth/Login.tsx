...
const Login: React.FC = () => {
  ...
  const errorDisplayed = useRef<boolean>(false);

  // Effect to handle error message display
  useEffect(() => {
    if (errorMessage) {
      console.log('Error message set:', errorMessage);
      errorDisplayed.current = true;

      // Focus on the form to ensure the error message is visible
      if (formRef.current) {
  ...
  const handleInputChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // Only clear error message if there is one and user is typing
    if (errorMessage) {
      setErrorMessage('');
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (axiosError.code === 'ERR_NETWORK') {
        ...
        errorMsg = responseData?.error || responseData?.message || 'Invalid credentials';
      } else if (axiosError.request) {
        // The request was made but no response was received
        errorMsg = 'No response from server. Please try again later.';
      } else {
        // Something happened in setting up the request that triggered an Error
        errorMsg = axiosError.message || 'Login failed';
      }
    ...
    <div className="min-h-screen flex items-center justify-center bg-gray-50 py-12 px-4 sm:px-6 lg:px-8">
      <div className="max-w-md w-full space-y-8">
        ...
        <form ref={formRef} className="mt-8 space-y-6" onSubmit={handleSubmit}>
          {errorMessage ? (
            <div className="rounded-md bg-red-50 p-4 mb-4 border-2 border-red-500 animate-pulse">
              <div className="flex">
                ...
                <div className="ml-3">
                  <h3 className="text-sm font-bold text-red-800">{errorMessage}</h3>
                </div>
              </div>
            </div>
          ) : null}
